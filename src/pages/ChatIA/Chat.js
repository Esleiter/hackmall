...
import ProductsGpt from "./ProductsGpt";
import { useState } from "react";
import { json } from "react-router-dom";

const Chat = () => {
  const { fetch, isLoading } = useFetchIa();
  const [arrayPrompt, setArrayPrompt] = useState([]);
  const { handleSubmit, values, handleChange } = useFormik({
    ...
    onSubmit: async ({ prompt }) => {
      ...
      console.log('xxxx', str.substring(2))
      // const arr = JSON.parse(`${str.substring(2)}`);
      setArrayPrompt(str);
    },
  });
  return (
    <>
      ...
      <div>
        <ProductsGpt arrayPrompt={arrayPrompt}/>
      </div>
    </>
  );
};
